...
export function HowPermaMindWorksContent() {
  ...
  return (
    <div className="space-y-6 overflow-y-auto px-4 pb-6 text-sm leading-6 text-muted-foreground">
      ...
      <section><h3 className="font-semibold text-foreground">{ar ? "بياناتك ملكك" : "You own your data"}</h3><p className="mt-1">{ar ? "يمكنك:" : "You can:"}</p><ul className="mt-2 list-disc space-y-1 pl-5"><li>{ar ? "تصدير ذاكرتك" : "Export your memory"}</li><li>{ar ? "استيراد النسخ الاحتياطية" : "Import backups"}</li><li>{ar ? "استخدام مفتاح API الخاص بك" : "Use your own AI API key"}</li><li>{ar ? "تعطيل النسخ الاحتياطي الدائم مستقبلاً" : "Disable future permanent backups at any time"}</li></ul><p className="mt-2">{ar ? "تعطيل النسخ الاحتياطي لا يحذف الملفات المرفوعة مسبقًا على Arweave." : "Disabling backups does not delete uploads already stored on Arweave."}</p></section>
    </div>
  );
}
