...
import { ArrowLeft, Brain, ChevronRight, MessageSquare, Search } from "lucide-react";
...
export function MemoryExperience({ conversations, onOpenConversation, initialConversationId }: MemoryExperienceProps) {
  ...
  if (selected) return (<main className="mx-auto h-full max-w-5xl overflow-y-auto p-4 pt-14 pb-8 sm:p-6 sm:pt-8 md:p-10"><button type="button" onClick={() => setSelectedId(null)} className="mb-6 inline-flex min-h-11 items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />Back to Memory</button><PageHeader eyebrow="Memory detail" title={selected.title} description="Remembered context from a saved conversation." actions={<StatusPill status={selected.permanentMemory ? "protected" : "neutral"} label={selected.permanentMemory ? "Protected" : "Saved locally"} />} /><div className="mt-6 grid gap-5 lg:grid-cols-[1fr_18rem]"><SurfaceCard title="Remembered content" description={selected.metadata ? "Conversation summary and available context" : "Available conversation content"}><div className="space-y-4 text-sm leading-7">{selected.metadata?.summary && <p>{selected.metadata.summary}</p>}{selected.messages.filter((message) => !message.isStreaming && message.content.trim()).map((message) => <div key={message.id} className="rounded-lg bg-muted/40 p-3"><p className="mb-1 text-xs font-medium text-muted-foreground">{message.role === "user" ? "You" : "PermaMind"}</p><p className="whitespace-pre-wrap">{message.content}</p></div>)}</div></SurfaceCard><SurfaceCard title="Provenance"><dl className="space-y-3 text-sm"><div><dt className="text-xs text-muted-foreground">Source</dt><dd>Saved conversation</dd></div><div><dt className="text-xs text-muted-foreground">Updated</dt><dd>{formatConversationTime(selected.updatedAt)}</dd></div></dl><div className="mt-5"><button type="button" onClick={() => onOpenConversation(selected.id)} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground hover:bg-primary/90">View conversation <ChevronRight className="size-4" /></button></div></SurfaceCard></div></main>);

  const cards: Array<{ conversation: Conversation; excerpt?: string }> = query
    ? results
        .map((result: MemorySearchResult) => ({ conversation: conversations.find((item) => item.id === result.conversationId), excerpt: result.snippet }))
        .filter((item): item is { conversation: Conversation; excerpt: string } => Boolean(item.conversation))
